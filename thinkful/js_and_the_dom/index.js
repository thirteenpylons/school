...
function render(contacts) {
    ...
    const contactsHTML = contacts.map(renderContact).join('');
    contactsElement.innerHTML = contactsHTML || 'No contacts available.';
}
...
function filterHandler() {
    ...
    filterElement.addEventListener('change', (event) => {
        ...
        const filteredContacts = selectedCity === '0' ? contacts : filterByCity(selectedCity);
...
function loadCities(contacts) {
    ...
    if (contacts.length > 0) {
        const cities = Array.from(new Set(contacts.map(contact => contact.address.city)));
        cities.forEach(city => {
...
function deleteButtonHandler() {
    document.getElementById('contacts').addEventListener('click', (event) => {
        if (event.target.classList.contains('deleteBtn')) {
            ...
            const contactId = parseInt(cardElement.dataset.id, 10);
